...
/*   Updated: 2023/03/23 13:39:25 by estruckm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_put_pixels(t_stack *stack)
{
	...
	while (i < stack->rows)
	{
		...
		{
			mlx_pixel_put(stack->mlx, stack->win, stack->data_x[i][j],
				stack->data_y[i][j], 0xFF0000);
			j++;
		}
		i++;
	}
...
void	ft_initialize_window(t_stack *stack)
{
	...
	stack->draw_loop = 0;
	stack->mlx = mlx_init();
	stack->win = mlx_new_window(stack->mlx, stack->stack_width,
			stack->stack_height, "My Window");
	stack->img_ptr = mlx_new_image(stack->mlx,
			stack->stack_width, stack->stack_height);
	stack->data_ptr = mlx_get_data_addr(stack->img_ptr,
			&stack->bpp, &stack->size_line, &stack->endian);
	host_get_clock_service(mach_host_self(),
		REALTIME_CLOCK, &stack->clock_service);
...
int	main(int argc, char **argv)
{
	...
	if (argc != 2)
	{
		ft_putendl_fd("Error! Provide two valid arguments!", 2);
		exit(0);
	}
	ft_input(argv[1]);
	stack->color_a = 0;
